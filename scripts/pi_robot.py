...
# HTML 博文模板
# ==========================================
ARTICLE_TEMPLATE = """    <main class="max-w-3xl mx-auto pt-28 pb-20 px-6">
        {hero_image}
...
# Load shell templates from _templates/ (single source of truth)
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_DIR = os.path.dirname(_SCRIPT_DIR)
_TEMPLATES_DIR = os.path.join(_PROJECT_DIR, "_templates")
...
_HEAD_TMPL = _load_template("head.html")
_NAV_TMPL = _load_template("nav.html")
_FOOTER_TMPL = _load_template("footer.html")
...
def generate_blog_html(title_cn, date_str, content_html, hero_img, slug):
    """生成博文 HTML 文件，使用 _templates/ 模板组装"""
    hero_html = ""
    ...
    # 1. 文章正文（ARTICLE_TEMPLATE 只有 <main> 部分）
    article_body = ARTICLE_TEMPLATE.format(
        title=title_cn,
        date=date_str,
    ...
    # 2. 组装 head + nav + article + footer（与 build.py 同源）
    canonical = f'https://pibizh.com/blog/{slug}.html'
    desc = f'{title_cn} - Pi Network 官方中文资讯，派币中文网第一时间翻译发布。'
    head_extra = f'''    <title>{title_cn} | 派币中文网</title>
    <meta name="description" content="{desc}">
    <link rel="canonical" href="{canonical}">
    <meta property="og:title" content="{title_cn} | 派币中文网">
    ...
    head_html = _HEAD_TMPL.replace("{{HEAD_EXTRA}}", head_extra).replace("{{BODY_CLASS}}", " pt-24")

    # 文章中间广告
    in_article_ad = '\n<!-- AdSense 文章中间广告 -->\n<ins class="adsbygoogle"\n     style="display:block; text-align:center;"\n     data-ad-layout="in-article"\n     data-ad-format="fluid"\n     data-ad-client="ca-pub-4800945095334481"\n     data-ad-slot="8883735919"></ins>\n<script>(adsbygoogle = window.adsbygoogle || []).push({});</script>'
    # 插入广告到文章内容之后
    article_body = article_body.replace('</div>\n    </main>', '</div>' + in_article_ad + '\n    </main>')
